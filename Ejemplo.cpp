...
void solicitarDatos(regAgenda *reg) //define un puntero a la variable reg
{
	...
	cout<< "Introducir CI:";
	cin>> reg->ci;
	cout<<"Nombrar su Gimnasio:";	
	do{
		gets(reg->gimnasio);
	}while(strlen(reg->gimnasio) == 0);
	reg->estado='N';
}

void mostrarRegistro(regAgenda reg, int nroReg)
{
	cout<<endl<< nroReg<<".-"<< reg.nombre<< "         "<<reg.ci<< "    "<< reg.gimnasio;
}
...
{int opcion;
	do{
		...
		cout<<"2.- Listado de clientes\n";
		cout<<"3.- Buscar cliente registrado\n";
